...
class ChatKeyboardView: UIView {
    ...
    private let kContentHeight: CGFloat = 350
    
    // MARK: - var lazy
        
    weak var delegate: ChatKeyboardViewDelegate?
    public var needHiddenToolBar = false
    ...
    fileprivate lazy var contentView: UIView = {
        let y = self.toolBarView.frame.maxY
...
extension ChatKeyboardView: ChatEmojiListViewDelegate {
    ...
    func emojiView(_ emojiView: ChatEmojiListView, DidFinish emotion: ChatEmoticon) {
        if (emotion.emojiCode?.count ?? 0 > 0) {
            chatTextView.insertText(emotion.emojiCode!)
        }else if (emotion.imgPath?.count ?? 0 > 0) {
    ...
    func emojiView(_ emojiView: ChatEmojiListView, DidFinish isSend: Bool) {
        if (isSend && chatTextView.text.count > 0) {
            sendChatMessage()
        }
    }
}
...
extension ChatKeyboardView {
    ...
    @objc func keyboardWillHide(_ noti: NSNotification) {
        ...
        let option = userInfo["UIKeyboardAnimationCurveUserInfoKey"] as! Int
        var changedY = kScreenH - self.toolBarHeight - self.contentHeight
        if needHiddenToolBar {
            changedY = kScreenH
        }
        UIView.animate(withDuration: duration, delay: 0, options: UIView.AnimationOptions(rawValue: UIView.AnimationOptions.RawValue(option)), animations: {
            self.frame = CGRect(x: 0, y: changedY, width: kScreenW, height: self.toolBarHeight + self.contentHeight)
...
extension ChatKeyboardView {
    ...
    func restToolbarContentHeight(_ isRest: Bool = false) {
        var changedY = kScreenH - self.toolBarHeight - contentHeight
        if (isRest) {
            if isShowEmoji {
                isShowEmoji = false
            }
        ...
        UIView.animate(withDuration: 0.25, delay: 0, options: .curveEaseOut, animations: {
            self.contentView.frame = CGRect(x: 0, y: self.toolBarView.frame.maxY, width: kScreenW, height: self.contentHeight)
            self.emojiListView.frame = self.contentView.bounds
            self.moreMenuView.frame = self.contentView.bounds
//            self.frame = CGRect(x: 0, y: changedY, width: kScreenW, height: self.toolBarHeight + self.contentHeight)
            if (isRest && self.needHiddenToolBar) {
                self.frame = CGRect(x: 0, y: changedY, width: kScreenW, height: 0)
            } else {
                self.frame = CGRect(x: 0, y: changedY, width: kScreenW, height: self.toolBarHeight + self.contentHeight)
            }
...
extension ChatKeyboardView {
    ...
    func changeKeyboardHeight(_ isClear: Bool = false, height: CGFloat) {
        ...
        toolBarView.frame = CGRect(x: toolBarView.frame.origin.x, y: 0, width: toolBarView.frame.width, height: toolBarHeight)
        ...
        bottomLineView.frame = CGRect(x: 0, y: toolBarView.frame.height - kLineHeight, width: kScreenW, height: kLineHeight)
        ...
        if (isShowKeyboard) {
            ...
            let changedY = kScreenH - keyboardHeight - toolBarHeight
            self.frame = CGRect(x: 0, y: changedY, width: kScreenW, height: toolBarView.frame.height + contentView.frame.height)
        }else {
            let changedY = kScreenH - (toolBarView.frame.height + contentView.frame.height)
            self.frame = CGRect(x: 0, y: changedY, width: kScreenW, height: toolBarView.frame.height + contentView.frame.height)
        }
