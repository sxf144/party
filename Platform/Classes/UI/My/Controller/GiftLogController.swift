...
class GiftLogController: BaseController {
    
    let CellHeight = 72.0
    var dataList: GiftLogModel = GiftLogModel()
...
extension GiftLogController {
    
    func getGiftLogs(pageNum:Int64, pageSize:Int64) {
        let monthStr = currentDate.ls_formatterStr("yyyy-MM")
        NetworkManager.shared.getGiftLogs(pageNum, pageSize: pageSize, month: monthStr) { resp in
...
// UITableView 代理
extension GiftLogController: UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "GiftLogCell", for: indexPath) as! GiftLogCell
